...
function getRowStatus(dateStr: string, employee: Employee, todayStr: string): RowStatus {
          ...
          <div className="bg-card border border-border rounded-xl p-5 shadow-sm shrink-0">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="grid grid-cols-2 gap-x-10 gap-y-3.5 flex-1">
                ...
                {/* Row 1: Name | PESEL */}
                <div>
                  <p className="text-xs text-muted-foreground mb-0.5">Imię i Nazwisko</p>
                ...
                <div>
                  ...
                  <p className="text-sm font-mono text-foreground tracking-wider">{employee.pesel || '—'}</p>
                </div>
                ...
                <div>
                  <p className="text-xs text-muted-foreground mb-0.5">Godziny pracy</p>
                  <p className="text-sm font-medium text-foreground tabular-nums">{employee.startHour}&thinsp;–&thinsp;{employee.endHour}</p>
                </div>

                {/* Row 4: Firma (left only) */}
                <div>
                  <p className="text-xs text-muted-foreground mb-0.5">Firma / Byt prawny</p>
            ...
            <div className="mt-4 pt-4 border-t border-border flex items-center gap-2">
              ...
              <select
                ...
                onChange={(e) => setTimesheetPeriod(parseInt(e.target.value, 10), timesheetYear)}
              ...
              <select
                value={timesheetYear}
                onChange={(e) => setTimesheetPeriod(timesheetMonth, parseInt(e.target.value, 10))}
                className="h-8 px-2 rounded-lg border border-input bg-background text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 transition appearance-none cursor-pointer"
              >
          ...
          <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden">
            ...
            <table className="w-full text-sm table-fixed">
              ...
              <thead>
                <tr className="border-b border-border bg-muted/50">
                  <th className="text-center px-3 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Dz.</th>
                  <th className="text-center px-3 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Tyg.</th>
                  <th className="text-center px-3 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Godziny pracy</th>
                  <th className="text-center px-3 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Nom.</th>
                  <th className="text-center px-3 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Real.</th>
                  <th className="text-center px-3 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wide">Status</th>
                </tr>
              </thead>
            </table>
            ...
            <div className="h-[calc(100vh-460px)] overflow-y-auto pb-8 scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-transparent">
              <table className="w-full text-sm table-fixed">
                ...
                <tbody>
                  {days.map(({ d, dateStr, weekend, rowStatus, isToday, isFuture }, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-3 py-2.5 text-center">{getDayOfWeek(timesheetYear, timesheetMonth, d)}</td>
